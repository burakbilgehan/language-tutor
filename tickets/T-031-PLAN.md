# T-031 Implementation Plan; Content language isolation
...
pre-existing collision bug). (4) Scope = comprehensive: "we can't show Turkish
...
effective status = mismatch ? "pending" : status so the list UI shows "Prepare".
...
- **Display gate** (`src/core/roadmap.ts` / `overview.ts`): when
  ...
  "Not prepared in this language; regenerate" placeholder instead of raw Turkish
  titles. This is what honors "we can't show Turkish to an English person."
...
warning at the change point: "cached content doesn't exist in this language yet;
it will be regenerated as you open it; there's a regen button for the curriculum."
Content is NOT deleted; return to tr restores exactly (map keys intact).
...
setError(e.message) }`. `"Profil yok"` (No profile) fires when NO profile exists -> no
...
- Scope: **top + mid tier only** (sweep ranking). Bottom-tier 400s ("response
  required" etc.) are code-bug-only, out of first pass; noted, not fixed.
- Codes to cover: profile_missing, curriculum_not_ready, not_found
  (kanji/word/topic/lesson/card/exercise/job; parameterized), llm_unconfigured
...
3. Roadmap shows en titles or "not prepared" placeholder, NOT raw Turkish.
